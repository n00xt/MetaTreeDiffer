...
public final class HierarchicalAgglomerativeClusterer {
    ...
    private double[][] computeDissimilarityMatrix() {
        dmatrix = new double[experiment.getNumberOfObservations()][experiment.getNumberOfObservations()];
        ...
        for (int i = 0; i < dissimilarityMatrix.length; i++) {
            for (int j = 0; j < dissimilarityMatrix[i].length; j++) {
                dmatrix[i][j] = dissimilarityMatrix[i][j];
            }
        }
